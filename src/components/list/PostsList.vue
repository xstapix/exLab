<script setup>
  ...
  const handlerCheck = (e) => {
    document.querySelector(`.${e.target.classList[1]}`).classList.toggle('post_favorite_check')
    document.querySelector(`.${e.target.classList[1]}B`).classList.toggle('post_checked')
  }
</script>

<template>
  <div v-if="postsStore.data.posts" class="postsBody">
    <div v-for="post in postsStore.data.posts" class="post DF" :class="`pac${post.id}B`">
